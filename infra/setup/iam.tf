...
data "aws_iam_policy_document" "tf_backend" {
  statement {
    effect    = "Allow"
    actions   = ["s3:GetObject", "s3:ListBucket", "s3:HeadObject", "ec2:DescribeAvailabilityZones", ]
    resources = ["arn:aws:s3:::${var.tf_state_bucket}"]
  }

  statement {
    effect  = "Allow"
    actions = ["s3:GetObject", "s3:PutObject", "s3:DeleteObject", "s3:HeadObject", "s3:ListBucket", "ec2:DescribeAvailabilityZones", ]
    resources = [
